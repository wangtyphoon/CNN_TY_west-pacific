...
def interpolate_data(csv, bst, reference_time):
    ...
    for i in range(length):
        sat_time = datetime.strptime(str(csv['Time'][i]), "%Y-%m-%d %H:%M:%S")
        bst_old = datetime.strptime(str(bst['Time'][count]), "%Y-%m-%d %H:%M:%S")
        bst_new = datetime.strptime(str(bst['Time'][count + 1]), "%Y-%m-%d %H:%M:%S")
        ...
        if bst_new > sat_time >= bst_old:
        ...
        elif sat_time > bst_new and count<(end-1):
            count += 1
            bst_old = datetime.strptime(str(bst['Time'][count]), "%Y-%m-%d %H:%M:%S")
            bst_new = datetime.strptime(str(bst['Time'][count + 1]), "%Y-%m-%d %H:%M:%S")
            ...
            csv['wind_speed'][i] = bst['wind_speed'][count + 1]
            csv['pressure'][i] = bst['pressure'][count + 1]
            csv['lat'][i] = bst['lat'][count + 1]
            csv['lon'][i] = bst['lon'][count + 1]
    return csv
