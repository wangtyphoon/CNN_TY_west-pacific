...
# 使用 NumPy 執行線性回歸
coefficientsT = np.polyfit(x, test_error, 1)
polynomialT = np.poly1d(coefficientsT)
coefficientsN = np.polyfit(x, train_error, 1)
polynomialN = np.poly1d(coefficientsN)
# 繪製誤差分布
plt.figure(figsize=(8, 6))
plt.plot(x,test_error,color="red",label="test")
plt.plot(x,train_error,color="blue",label="train")
plt.plot(x, polynomialT(x), label='Test Regression Line', color='black')
plt.plot(x, polynomialN(x), label='Train Regression Line', color='brown')

plt.legend()
plt.ylabel('error')
plt.title('Train process')
plt.grid()
plt.savefig("round3.png",dpi=400)
plt.show()

print("訓練結束")  
regressor.save_weights('./effM_weights(ws)(round3).h5')
