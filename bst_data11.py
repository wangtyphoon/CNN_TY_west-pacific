...
for sub_folder in subfolders:
    ...
    df = pd.read_csv("bst/"+sub_folder_path[-5:]+".csv")
    ...
    for i, file_name in enumerate(image_files):
        ...
        # df_ws['wind_speed'][randow_number[count]-1] = df['wind_speed'][rating_number]
        # df_ws['pressure'][randow_number[count]-1] = df['pressure'][rating_number]
        # df_ws['lon'][randow_number[count]-1] = df['lon'][rating_number]
        # df_ws['lat'][randow_number[count]-1] = df['lat'][rating_number]
        # df_ws['time'][randow_number[count]-1] = df['Time'][rating_number]
        df_ws['tag'][randow_number[count]-1] = "bst"
        df_ws['ordinal'][randow_number[count]-1] = str(sub_folder)

        count += 1
        rating_number += 1
df_ws.to_csv("typhoon1.csv",index=False)
